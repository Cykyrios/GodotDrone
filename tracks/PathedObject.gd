...
func set_spacing(space: float):
	spacing = space
	update_multimesh()


func set_spline_points(points):
	var points_array = []
	points = points.replace("\n", "")
	var string_array = points.split("]")
	...
	for string in string_array:
		...
		var vector_string = string.split(")")
...
func update_multimesh():
	...
	if spacing >= curve_length:
		...
		multimesh.instance_count = curve_length / spacing
